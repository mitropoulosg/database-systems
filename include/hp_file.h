...
typedef struct {
    ...
    //heap file or hash 
    char file_type[10];
    //Θέση του owner block στον buffer
    void* owner_block_buffer_pos;
    // id του τελευταιου block του αρχειου hp
    int last_block_id;
    // αριθμος εγγραφων που χωραει σε καθε μπλοκ του αρχειου
    int rec_capacity;
} HP_info;

// τα μεταδεδομένα του block (αποθηκεύεται στο τελος του block)
typedef struct {
    // αριθμος εγγραφών στο μπλοκ
    int num_of_records;
} HP_block_info;
